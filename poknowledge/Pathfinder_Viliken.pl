...
sub EVENT_ITEM {
  ...
  if ($turnedin < $maxturnedin) {
    while(quest::handin({ 29131 => 1 })) { # Item(s): Spectral Parchment
      quest::summonitem(quest::ChooseRandom(59588, 26931, 26929, 26930, 21626)); #Level 63 or 64 Ranger spell, PoP (Frozen Wind, Nature's Rebuke, Spirit of the Predator, Brushfire)
      $turnedin++;
  ...
  if($turnedin >= 1) {
    ...
    quest::say("Use this wisely and with great respect for the power that has originated it. The primordial arcane powers of the divine worlds are not to be disrespected.");
  }
}